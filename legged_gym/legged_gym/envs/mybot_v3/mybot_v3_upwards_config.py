...
class MyBotV3UpwardsCfg( LeggedRobotCfg ):
    task_name = 'mybot_v3_upwards'
    class init_state( LeggedRobotCfg.init_state ):
        pos = [0.0, 0.0, 0.34] # x,y,z [m]
        # Foot positions relative to the base, order is [FL, FR, RL, RR].
        # Computed from the mybot_v3 URDF at the default standing joint angles.
        rel_foot_pos = [[0.2438, 0.2438, -0.2354, -0.2354], # x
                        [0.1667, -0.1667, 0.1667, -0.1667], # y
                        [-0.332, -0.332, -0.332, -0.332]] # z

        # Link lengths + hip offsets used by the analytic IK (init-state randomisation).
        # Derived from the mybot_v3 URDF (thigh = 0.22 m, calf = 0.226 m).
        HIP_LINK_LENGTH = 0.1047   # lateral hip-joint -> leg-plane distance [m]
        THIGH_LINK_LENGTH = 0.22   # [m]
        CALF_LINK_LENGTH = 0.226   # [m]
        # Hip offsets in the IK-internal [FR, FL, RR, RL] order:
        ik_hip_x_offsets = [0.2398, 0.2398, -0.2394, -0.2394]
        ik_hip_y_offsets = [-0.062, 0.062, -0.062, 0.062]
    ...
    class domain_rand ( LeggedRobotCfg.domain_rand ):
        ...
        class ranges():
            ...
            min_robot_pos = [-0.0, 0.0, 0.0] # min robot pos [m] (relative to base height)
            max_robot_pos = [-0.0, 0.0, 0.7] # min robot pos [m] (relative to base height)
...
class MyBotV3UpwardsCfgPPO( LeggedRobotCfgPPO ):
    ...
    class runner( LeggedRobotCfgPPO.runner ):
        run_name = ''
        experiment_name = 'mybot_v3_upwards'
